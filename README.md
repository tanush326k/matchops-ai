...
[![Gemini AI](https://img.shields.io/badge/Gemini_AI-Powered-8E75B2?style=for-the-badge&logo=googlegemini&logoColor=white)](https://ai.google.dev)
...
```mermaid
graph TD
    A["🎤 User Query Input"] --> B["🔍 Multi-Intent Classifier"]
    B --> C["🛡️ Role Authorization & Guardrails"]
    C --> D{"Authorized?"}
    D -->|No| E["⚠️ Polite Refusal + Permitted Actions"]
    D -->|Yes| F["📡 Context Grounder — Live Telemetry"]
    F --> G["🧮 Decision Planner — Rules Engine"]
    G --> H["📊 Confidence Diagnostics Scorer"]
    H --> I["💾 Session Memory — 4-Turn History"]
    I --> J{"Gemini API Available?"}
    J -->|Yes| K["✨ Gemini Structured JSON Generation"]
    J -->|No| L["🔧 High-Fidelity Local Fallback Engine"]
    K --> M["✅ Pydantic Schema Validation"]
    L --> M
    M --> N["📋 Structured Response — Plans, Reasoning, Confidence"]
```
...
graph TB
    subgraph Client["🖥️ Frontend — React + TypeScript + Vite"]
        LP["Landing Page<br/>Role Selector"]
        DB["Dashboard<br/>Mission Control"]
        SM["Stadium Map<br/>SVG Overlays"]
        CP["Copilot Panel<br/>AI Interface"]
        KPI["KPI Cards<br/>Sparkline Trends"]
    end

    subgraph Server["⚡ Backend — FastAPI + Python"]
        API["REST API<br/>Endpoints"]
        SIM["Simulation Engine<br/>5s Tick Loop"]
        TEL["Telemetry Store<br/>In-Memory Database"]
    end
...
[![Live Demo](https://img.shields.io/badge/🚀_Live_Demo-matchops--ai.onrender.com-000000?style=for-the-badge)](https://matchops-ai.onrender.com)
